...
Smart contracts expose two layers of machine-readable information: the **ABI** (what functions exist and their Solidity types) and **NatSpec** (developer-facing documentation embedded in source code). Neither serves end users [TODO:NatSpec Notice?].
...
- **Contract context** —[TODO] title, description, origin story, category, tags, links, risks, audits
...
- **Input guidance** — how parameters should be collected (ENS resolution [TODO], address book, dropdown, slider)
...
| `factory`       | `object` | Factory binding (alternative to `address` for factory-deployed contracts)[TODO] |
...
Each function is keyed by its ABI name [TODO: overloaded functions?]. Metadata enriches parameters with labels, descriptions, and semantic type annotations:
...
Display hints are semantic annotations that tell consumers what a value _represents_, not how to style it. A `uint256` in the ABI carries no meaning beyond "256-bit unsigned integer." Display hints bridge that gap:
...
| `{ type: "basis-points" }` [TODO:custom format all string?]   | Value in basis points (1/100th of a percent) |
...
**Merge semantics:** The merge is _shallow per top-level key within each section_. When a contract defines a function that also exists in an included interface, the contract's entire function object replaces the interface's — there is no deep merge of `params`, `returns`, or other nested fields. This means if you override a function, you must re-declare everything you want to keep (params, returns, displayHints, etc.).
